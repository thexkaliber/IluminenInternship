...
img = Image.open("Grey Scale - large 16 bit.bmp") #Fetch the image
...
img_matrix = np.where((img_matrix==0) | (img_matrix==255),img_matrix, 255) #Replace all pixels which aren't Black or White to White
print('\nModified image data (Whiten):\n',img_matrix)


print('\nSaving as a 1bit BMP file...')  #Save the image as a Black and White BMP File
img_blacked = Image.fromarray(img_matrix).convert('1')
img_blacked.save('Greyscale_white.bmp')
print('Saved file.')